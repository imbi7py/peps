...
present and ``False``.  The platform is compatible if
``_manylinux.manylinux2`` is ``True`` or if that module or its
attribute is missing and the system uses ``glibc`` 2.12.
